...
<Warning>
  <u>Warning to Customers</u>: Some pages and sections in 'Features and Pages' are currently not added, or the information is not fully complete. Please bear with us while we get this sorted. If you require assistance with individual sections of unfinished documentation pages ,contact support.
</Warning>
...
<Danger>
  **\[03/2026\] Important:** We are aware of an issue with setting up both Discord and Roblox permissions, saying they have not saved to the main database; this is **not the case,** and they work as intended and are a frontend error. Please ignore the error message and continue as normal, and it will work. We are working on pushing out a fix as quickly as possible. 
</Danger>
...
<Tabs>
  <Tab title="Discord Role Permissions">
    ...
    Once all permissions have been set up for a role, they will appear with the `/permissions list `slash command, which you must ensure you set permissions for **your selected discord roles; **otherwise, they will be unable to access staff Discord features - student roles do not require this. 
  ...
  <Tab title="Roblox Role Permissions">
    ROBLOX permissions allow users to access the staff side of LuaSIMS instead of the student view and set your permissions based on your community rank, restricting certain features to maintain realism. To do this, run the slash command`/pemissions add roblox`and then follow the other prompts, in the example provided below.
    ...
    Once all permissions have been set up for a role, they will appear with the `/permissions list `slash command, which you must ensure you set permissions for **every single role, **otherwise they will be unable to access staff features - student ranks do not require this. 
...
This section is **work in progress, **as this is <u>not currently on the live version</u>, check back soon. 

<Danger>
  Roblox commands and additional features are not yet available for **LuaSIMS 2.1 **
</Danger>